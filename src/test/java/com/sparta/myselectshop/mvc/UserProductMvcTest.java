...
@WebMvcTest(
    controllers = {UserController.class, ProductController.class}, // 테스트할 controller
...
class UserProductMvcTest {
    private MockMvc mvc; // http 요청을 가짜로 날릴 수 있다.
    ...
    public void setup() {
        mvc = MockMvcBuilders.webAppContextSetup(context)
            .apply(springSecurity(new MockSpringSecurityFilter())) // 우리가 만들어준 filter 끼워넣기
            .build();
    }
    ...
    void test1() throws Exception {
        ...
        mvc.perform(get("/api/user/login-page"))
            .andExpect(status().isOk()) // Ok 예측
            .andExpect(view().name("login")) // login 예측
            .andDo(print());
    }
